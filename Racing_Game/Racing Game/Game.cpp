...
void game::init(const char* title, int posX, int posY, int width, int height)
{
    ...
    {
        window = SDL_CreateWindow("Racing Game", posX, posY, width, height, 0);
        isRunning = true;
...
void game::HandleEvents()
{
    ...
    if (!getPaused())
    {
        ...
        if ((keyState[SDL_SCANCODE_D] || keyState[SDL_SCANCODE_RIGHT]) && Player->posX < 480)
        {
            Player->move("right");
        }
        if ((keyState[SDL_SCANCODE_A] || keyState[SDL_SCANCODE_LEFT]) && Player->posX > 300)
        {
            Player->move("left");
        }
        if ((keyState[SDL_SCANCODE_S] || keyState[SDL_SCANCODE_DOWN]) && Player->posY < 530)
        {
            Player->move("down");
        }
        if ((keyState[SDL_SCANCODE_W] || keyState[SDL_SCANCODE_UP]) && Player->posY > 0)
        {
            Player->move("up");
        }
